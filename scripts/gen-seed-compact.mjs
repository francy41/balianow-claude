...
// La PK real es UUID → generamos un UUID DETERMINISTA por cada id demo, para
// conservar relaciones (evento→local) e idempotencia (mismo UUID en cada corrida).
const uuid = (seed) => {
  const h = createHash('md5').update('bailanow:' + seed).digest('hex')
...
sql += VENUES.map(v => `(${s(uuid('venue:'+v.id))},NULL,${s(v.name)},${s(VT.has(v.type) ? v.type : 'club')},${s(v.city)},${s(v.cover)},${s(v.avatar)},${n(v.rating)},${n(v.reviews)},${n(v.capacity)},${b(v.isOpen)},${b(v.isPremium)},${[1,2,3,4].includes(v.priceRange) ? v.priceRange : 2})`).join(',\n')
...
sql += ARTISTS.map(a => `(${s(uuid('artist:'+a.id))},NULL,${s(a.name)},${s(AT.has(a.type) ? a.type : 'dj')},${arr(a.genre)},${s(a.avatar)},${s(a.cover)},${s(a.city)},${s(a.country)},${n(a.rating)},${n(a.reviews)},${n(a.followers)},${n(a.priceFrom)},${b(a.isVerified)},${b(a.isPremium)})`).join(',\n')
sql += `\nON CONFLICT (id) DO NOTHING;\n`
...
sql += evs.map(e => `(${s(uuid('event:'+e.id))},${s(e.title)},${venueIds.has(e.venueId) ? s(uuid('venue:'+e.venueId)) : 'NULL'},${s(e.venueName)},${s(e.city)},${s(e.date.slice(0,10))},${s(e.time)},${s(e.cover)},${arr(e.category)},${n(e.price)},${n(e.capacity)},${n(e.attending)},${b(e.isFeatured)},${b(e.isPremium)},NULL)`).join(',\n')
sql += `\nON CONFLICT (id) DO NOTHING;\n`
